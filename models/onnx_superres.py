...
def get_session(onnx_path: Path, use_cpu: bool = False) -> ort.InferenceSession:
    ...
    sess_opts.intra_op_num_threads = 32
...
def sr_image(session: ort.InferenceSession, img: np.ndarray, scale: int, window_size: int, tile_size: int, tile_pad: int) -> np.ndarray:
...
def main():
    ...
    for img_path in imgs:
        ...
        sr = sr_image(session, img, scale=args.scale, window_size=args.window_size, tile_size=args.tile_size, tile_pad=args.tile_pad)
        save_name = f"{img_path.stem}_x{args.scale}.png"
        save_image(sr, out_dir / save_name)
        print(f"[done] {img_path.name} -> {save_name}")
