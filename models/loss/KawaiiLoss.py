...
from ArtifactPerceptualLoss import ArtifactPerceptualLoss
...
class KawaiiLoss(nn.Module):
    ...
    Args:
        artifact_detector (nn.Module): 预训练好的伪影检测分类器。
    ...
    def __init__(self, 
        ...
        # 5. 伪影惩罚损失
        self.art_loss = ArtifactPerceptualLoss(artifact_detector=artifact_detector).to(device)
    ...
    def forward(self, sr_img: torch.Tensor, hr_img: torch.Tensor, fake_logits: torch.Tensor):
        ...
        if self.lambda_art > 0:
            loss_art = self.art_loss(sr_img)
            loss_components['art'] = loss_art.item()
            total_loss += self.lambda_art * loss_art
...
if __name__ == '__main__':
    ...
    class MockArtifactDetector(nn.Module):
        def __init__(self):
            ...
            self.main = nn.Sequential(
                nn.Conv2d(3, 8, 3, 1, 1), nn.LeakyReLU(0.2, True), nn.MaxPool2d(2),
                nn.Conv2d(8, 16, 3, 1, 1), nn.LeakyReLU(0.2, True), nn.MaxPool2d(2),
                nn.Flatten(),
                nn.Linear(16 * (IMG_SIZE // 4)**2, 2)
            )
    ...
    artifact_detector = MockArtifactDetector().to(DEVICE)
    
    # 将伪影检测器传入生成器损失函数
    g_loss_fn = KawaiiLoss(artifact_detector=artifact_detector, device=DEVICE)
